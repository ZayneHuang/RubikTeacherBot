...
from environments import cube3_full
import numpy as np
...
class CubeSolver:
    def __init__(self,Fformula,Oformula,Pformula):
        ...
        self.nnet = nnet_utils.load_nnet('saved_models/cube3_cross_blind/target/model_state_dict.pt', self.env.get_nnet_model(),device=self.device)
...
def getResults(state):
    ...
    cubesolver=CubeSolver('formula/F2L.txt','formula/OLL.txt','formula/PLL.txt')

    state = np.array(state)
    FE2STATE=np.array([6, 3, 0, 7, 4, 1, 8, 5, 2, 15, 12, 9, 16, 13, 10, 17, 14, 11, 24, 21, 18, 25, 22, 19, 26, 23, 20, 33, 30, 27, 34, 31, 28, 35, 32, 29, 38, 41, 44, 37, 40, 43, 36, 39, 42, 51, 48, 45, 52, 49, 46, 53, 50, 47])
...
if __name__=='__main__':
    ...
    current_state = [47,12,20,19,4,52,11,5,44,24,34,26,37,13,30,35,7,0,8,3,33,1,22,41,17,16,15,29,10,45,39,31,23,9,50,18,38,46,42,43,40,48,27,25,6,53,28,2,32,49,21,36,14,51]
    print(current_state)
